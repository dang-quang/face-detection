...
@property FaceLandmarkPose *faceLandmarkPose;
...
- (void)detectImage:(UIImage *)image {
    ...
    std::vector<FaceLandmarkPoseResult> faceLandmarkPoseResult;
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        ...
        } else if(weakSelf.USE_MODEL == W_FACE_LANDMARK_POSE){
            weakSelf.imageView.image = [weakSelf drawFaceLandmarkPose:weakSelf.imageView image:image faceLandmarkPose:faceLandmarkPoseResult];
        }
        // include draw
...
- (void)createModel {
    if (!self.yolo && self.USE_MODEL == W_YOLOV5S) {
        ...
        //self.faceLandmark = new FaceLandmark(self.USE_GPU);
         self.faceLandmarkPose = new FaceLandmarkPose(self.USE_GPU);
...
- (UIImage *)drawFaceLandmarkPose:(UIImageView *)imageView image:(UIImage *)image faceLandmarkPose:(std::vector<FaceLandmarkPoseResult>)faceLandmarkPose {
    UIGraphicsBeginImageContext(image.size);
    ...
    for (int i = 0; i < faceLandmarkPose.size(); i++) {
        FaceLandmarkPoseResult faceLandmarkPosex = faceLandmarkPose[i];
        srand(i / 106 + 2020);
        UIColor *color = [UIColor colorWithRed:rand()%256/255.0f green:rand()%256/255.0f blue:rand()%255/255.0f alpha:1.0f];
        CGContextSetFillColorWithColor(context, [color CGColor]);
        //TODO:
        CGContextFillRect(context, CGRectMake(faceLandmarkPosex.bbox.x1 - 2, faceLandmarkPosex.bbox.y1 - 2, 5, 5));
    }
